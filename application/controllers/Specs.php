...
class Specs extends MY_Controller
{
	...
	function __construct()
	{
		...
		// $this->uploadToS3->uploadToTemp();
		// $this->uploadToS3->uploadToBucket($origPath, $newPath, $deleteOrigPathAfterChange = FALSE);
...
//					  'user_id' => $aux[2],
                        ...
                        // cleaning up the url_dir
			//echo '<pre> DEBUGGING TEST line: '.__LINE__.' file: '.__FILE__.' <br />';
			//print_r($aux_d['url_dir']);
			//echo '</pre>';	
						$memoTagImgUrl = str_replace($_SERVER['DOCUMENT_ROOT'].'/', '', $aux_d['url_dir']);
			//echo '<pre> DEBUGGING TEST line: '.__LINE__.' file: '.__FILE__.' <br />';
			//print_r($memoTagImgUrl);
			//echo '</pre>';	
						$memoTagImgUrl = $this->fileuploadtos3->convertLegacyImgSrcToS3( $memoTagImgUrl);
			//echo '<pre> DEBUGGING TEST line: '.__LINE__.' file: '.__FILE__.' <br />';
			//print_r($memoTagImgUrl);
			//echo '</pre>';	
						...
						$this->data['list_files']['tbody'] .= "
				<tr>
					<td><a href='" . $memoTagImgUrl . "' target='_blank'><i class='fa fa-file btnViewFile' aria-hidden='true'></i></a></td>
					<td>" . $aux_d['date'] . "</td>
					" . ($this->hasPermission('specs', 'edit') ? "<td><i class='fa fa-times-circle delete_temp_url' aria-hidden='true'></i></td>" : '') . "
				</tr>";
					}
				}
		...
		if(true){
			...
			if(array_key_exists('shared_files', $selectedRow) and $selectedRow['shared_files']){
				...
				foreach ($arr as $i) { // Loop through each of the files
					...
					$tmp_file= $i->url_dir; // $tmp_filebecomes the Url
					if (strpos($tmp_file, 'temp')) {
						//$extension = explode('.', str_replace(site_url() . $this->file_directory->temp_folder, '', $tmp_file)); // [0] name / [1] extension .jpg or .pdf (whatever)
						$extension = $this->fileuploadtos3->getImageFileExtension($tmp_file);
						$filename = $given_spec_id . '-' . url_title($i->date, '-', true) . '-' . $n . '.' . $extension;
						...
						// echo '<pre> DEBUGGING TEST for $_new_location line: '.__LINE__.' file: '.__FILE__.' <br />';
						// print_r($_new_location);
						// echo '</pre>';
		                // echo '<pre> DEBUGGING TEST for $new_location line: '.__LINE__.' file: '.__FILE__.' <br />';
						...
						// Save new location
						// rename(str_replace(site_url(), '', $tmp_file), $new_location);
				        ...
				        /*  ------------------------
				         ...
				         $this->fileuploadtos3->SendUploadedTempFileToS3($tmp_file, $new_location);

				         // PKL Convert the new file location ($new_location_db) to S3 URL for Database insertion
				         $S3_location = $this->fileuploadtos3->convertLegacyImgSrcToS3($new_location);
                         $new_location_db = $S3_location;


						// Savecurrent file to S3 bucket
						// echo "<pre> (".__LINE__.") Savecurrent file to S3 bucket";
						// print_r( str_replace(site_url(), '', $tmp_file) );
						// print_r(  $new_location);
						// echo "</pre>"; 
					} else {
						// Existing file, don't relocate the file
						$new_location = str_replace(site_url(), '', $tmp_file);
					}

					$ret = array(
					  'related_id' => $given_spec_id,
					  'url_dir' => $new_location_db,
	...
	*/
	...
	public function edit_style()
	{
		...
		//echo '</pre>';	
					...
					$data['list_files']['tbody'] .= "
					<tr>
						<td><a href='" . $memoTagImgUrl . "' target='_blank'><i class='fa fa-file btnViewFile' aria-hidden='true'></i> " . ($isOther ? $aux_d['descr'] : $aux_d['category_name']) . "</a></td>
						<td>" . $aux_d['date'] . "</td>
						" . ($this->hasPermission('specs', 'edit') ? "<td><i class='fa fa-times-circle delete_temp_url' aria-hidden='true'></i></td>" : '') . "
					</tr>";
				}
			...
			//echo "<pre DEBUGGING TEST (pic_big_url) line: ".__LINE__." file: ".__FILE__.">";
            //print_r($data['info']['pic_big_url']);
            //echo "</pre>";
			...
			if (!$data['isNew'] && !is_null($data['info']['pic_big_url'])) {
			  ...
			  $data['info']['pic_big_url'] = $this->fileuploadtos3->convertLegacyImgSrcToS3( $data['info']['pic_big_url']);
...
//                $data['info']['pic_big_url'] = $this->file_directory->image_src_path('load', 'digital_styles') . $given_style_id . '.jpg';
			...
			foreach ($data['items'] as $key => $value) {
				if (!is_null($data['items'][$key]['pic_big_url'])) {
				  $_img_url = $data['items'][$key]['pic_big_url'];
				  // remove DOCUMENT_ROOT from the url in preparation for S3
				  $_img_url = str_replace($_SERVER['DOCUMENT_ROOT'], '',  $_img_url);
                  ...
                  */
				  $data['items'][$key]['pic_big_url'] = $this->fileuploadtos3->convertLegacyImgSrcToS3($_img_url);
...
//                    $data['items'][$key]['pic_big_url'] = $this->file_directory->image_src_path('load', 'digital_styles_items') . $data['items'][$key]['id'] . '.jpg';
        ...
        $ret['dev_feedback']['pic_big_url'] = 'data=>info=>pic_big_url: ' . $data['info']['pic_big_url'];
	...
	public function save_style()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			if ($this->input->post('change_showcase') === '1') {
				...
				$tmp_file = $this->input->post('pic_big_url');

				//echo "<pre> (".__LINE__.") _POST['pic_big_url'] ". $_POST['pic_big_url'] . " <br />";
				
				//echo "<pre> (".__LINE__.") tmp_file = $tmp_file  <br />";
				if (strpos($tmp_file, 'temp')) {
					// Is a new file!
					$file_folder_url = site_url() . $this->file_directory->temp_folder;

					//echo '<pre> DEBUGGING TEST line: '.__LINE__.' file: '.__FILE__.' <br />';
					//print_r($file_folder_url);
					//echo '</pre>';	

					//$extension = explode('.', str_replace( $file_folder_url, '', $tmp_file)); // [0] name / [1] extension .jpg or .pdf (whatever extension is)
					$extension = $this->fileuploadtos3->getImageFileExtension($tmp_file);
					$location_request = [
					  'status' => 'save',
					...
					if (file_exists($new_location)) {
						// Move existing file
						$_destination_for_replacement = str_replace('.' . $extension, '-' . url_title(date("Y-m-d h-i-sa")) . '.' . $extension, $new_location);
						rename($new_location, $_destination_for_replacement);
						// Savecurrent file to S3 bucket
						// echo "<pre> (".__LINE__.") Savecurrent file to S3 bucket";
						// print_r($new_location);
						// print_r( $_destination_for_replacement);
						// echo "</pre>";  
						// die();
					}
					...
					// echo " (".__LINE__.") tmp_file = $tmp_file  <br />";
					// echo " (".__LINE__.") new_location = $new_location  <br />";
				    
                    /*  ------------------------
					  Upload new assets with the new fileuploadtos3 class
					  fileuploadtos3::SendUploadedTempFileToS3($tmp_file, $new_location);
					  Both Params should be web relative paths
					  ------------------------ */
					$tmp_file        = str_replace('https://' . $_SERVER['HTTP_HOST'], '', $tmp_file);
					$new_location    = str_replace($_SERVER['DOCUMENT_ROOT'],'',$new_location);
					$new_location_db = str_replace($_SERVER['DOCUMENT_ROOT'],'',$new_location_db);

					//echo '<pre> DEBUGGING TEST for $tmp_file line: '.__LINE__.' file: '.__FILE__.' <br />';
					//print_r($tmp_file);
					//echo '</pre>';
					//echo '<pre> DEBUGGING TEST for $new_location line: '.__LINE__.' file: '.__FILE__.' <br />';
					//print_r($new_location);
                    ...
                    $new_location_db = $S3_location;
					//  --> rename(str_replace(site_url(), '/opuzen-efs/dev/opms/', $tmp_file), $new_location);
			...
			if ($given_style_type === constant('Digital') && $this->input->post('change_files_encoded') === '1') {
				...
				foreach ($arr as $i) { // Loop through each of the files
					...
					if ($i->category_id === $this->category_files_ids['memotags_picture']) {
						...
						if (strpos($tmp_file, 'temp')) {
							// Is a new file!
							//$extension = explode('.', str_replace(site_url() . $this->file_directory->temp_folder, '', $tmp_file)); // [0] name / [1] extension .jpg or .pdf (whatever extension is)
							$extension = $this->fileuploadtos3->getImageFileExtension($tmp_file);
							$filename = $given_style_id . '-' . url_title($i->category_id . '-' . $i->category_name . '-' . $i->date, '-', true) . '-' . $n . '.' . $extension;
							...
							$_new_location = $this->file_directory->image_src_path($location_request);
							// echo '<pre> DEBUGGING TEST for $_new_location : '.__LINE__.' file: '.__FILE__.' <br />';
							// print_r($_new_location);
							// echo '</pre>';
							$new_location = $_new_location . '/' . $filename;
							$new_location = str_replace("//", "/", $new_location);
							...
							// echo '<pre> DEBUGGING TEST for $tmp_file: '.__LINE__.' file: '.__FILE__.' <br />';
							// print_r($tmp_file);
							// echo '</pre>';
							// echo '<pre> DEBUGGING TEST for $new_location line: '.__LINE__.' file: '.__FILE__.' <br />';
							// print_r($new_location);
						    ...
						    /*  ------------------------
						     ...
						     Both Params should be web relative paths
						     ------------------------ */
                            // remove the document root from the $new_location
                            $tmp_file      = str_replace('https://' . $_SERVER['HTTP_HOST'],'',$tmp_file);
							$new_location  = str_replace($_SERVER['DOCUMENT_ROOT'],'',$new_location);
							$new_location  = str_replace('/files','/showcase/images',$new_location);
							// echo '<pre> DEBUGGING TEST for $tmp_file: '.__LINE__.' file: '.__FILE__.' <br />';
							// echo ' PREFIX: https://' . $_SERVER['HTTP_HOST'],' <br />';
							// print_r($tmp_file);
							// echo '</pre>';
							// echo '<pre> DEBUGGING TEST for $new_location line: '.__LINE__.' file: '.__FILE__.' <br />';
							// print_r($new_location);
			...
			if (!empty($item_ids_edited)) {
				...
				foreach ($item_ids_edited as $id) {
					...
					$tmp_file= $this->input->post('pic_big_url_' . $id);

					if (strpos($tmp_file, 'temp')) {
						//$extension = explode('.', str_replace(site_url() . $this->file_directory->temp_folder, '', $tmp_file));
						$extension = $this->fileuploadtos3->getImageFileExtension($tmp_file);
		                ...
		                $tmp_pic_big_url = $tmp_file= $this->input->post('pic_big_url_' . $id);
		                /*  ------------------------
		                 Create S3 objects from uploads with the new fileuploadtos3 class
		                 fileuploadtos3::SendUploadedTempFileToS3($tmp_file, $new_location);
		                 Both Params should be web relative paths
						 ...
						 // echo '<pre> DEBUGGING TEST for $tmp_file: '.__LINE__.' file: '.__FILE__.' <br />';
						 // print_r($tmp_pic_big_url);
						 // echo '</pre>';
						 // echo '<pre> DEBUGGING TEST for $new_location line: '.__LINE__.' file: '.__FILE__.' <br />';
						 // print_r($new_location);
						 // echo '</pre>';

		                 $this->fileuploadtos3->SendUploadedTempFileToS3($tmp_pic_big_url, $new_location); 
		                 //echo "<pre> " . __CLASS__ . " FUNCTION: " . __FUNCTION__ . " LINE: " . __LINE__ . " <br>";
		                 //echo '$tmp_pic_big_url: '. $tmp_pic_big_url ."<br>";
...
//	                    rename(str_replace(site_url(), '', $tmp_file), $new_location);
					...
					// echo "<pre> (".__LINE__.") styles_form.php";
					// print_r( $item_color_ids  );
					// print_r( $item_color_names);
					// echo "</pre>";  
					...
					if ($isNew) {
						...
						if (strpos($tmp_file, 'temp')) {
							...
							if (file_exists($new_location)) {
								// Move existing file
								$location_request['item_id'] = strval($id) . '-' . url_title(date("Y-m-d h-i-sa"));
								$altered_new_location = $this->file_directory->image_src_path($location_request);
								// echo '<pre> DEBUGGING TEST for $altered_new_location line: '.__LINE__.' file: '.__FILE__.' <br />';
								// print_r($altered_new_location);
								// echo '</pre>';
							  /*  ------------------------
							   Create S3 objects from uploads with the new fileuploadtos3 class
							   fileuploadtos3::SendUploadedTempFileToS3($tmp_file, $new_location);
							   Both Params should be web relative paths
							   ------------------------ */
							   $new_location    = str_replace($_SERVER['DOCUMENT_ROOT'],'',$altered_new_location);
							   $this->fileuploadtos3->SendUploadedTempFileToS3($tmp_file, $altered_new_location); 
...
//	                        rename(str_replace(site_url(), '', $tmp_file), $new_location);
		...
		// echo "<pre> (".__LINE__.") Savecurrent file to S3 bucket <br />";
		// print_r(  $tmp_file);
		// echo "<br /> New Location: <br />";
		// print_r(  $new_location);
		// echo "<br />JSON: <br />";
		// print_r(  $ret );
		// echo "</pre>";  	
				...
				if (!empty($arr)) {
					foreach ($arr as $r) {
						...
						<tr data-perc='" . $r['perc'] . "' data-content-id='" . $r['id'] . "' data-content-name='" . $r['name'] . "'>
							<td>" . $r['perc'] . "%</td>
							<td>" . $r['name'] . "</td>
							<td><i class='fa fa-trash btnDeleteRow pull-right' aria-hidden='true'></i></td>
						</tr>
						";
					}
				} else {
					...
					foreach ($arr as $r) {
						...
						if (count($files) > 0) {
							foreach ($files as $tmp_file) {
								$filesanchors .= "<a href='" . site_url() . str_replace(site_url(), '', $tmp_file) . "' target='_blank'><i class='fa fa-file btnViewFile' aria-hidden='true'></i></a> ";
							}
						}
						$data['tbody'] .= "
						<tr data-id='" . $r['id'] . "' data-rubs='" . $r['rubs'] . "' data-abrasion-limit-id='" . $r['abrasion_limit_id'] . "' data-abrasion-limit-name='" . $r['abrasion_limit_name'] . "' data-abrasion-test-id='" . $r['abrasion_test_id'] . "' data-abrasion-test-name='" . $r['abrasion_test_name'] . "' data-date-add='" . $r['date_add'] . "' data-visible='" . $r['visible'] . "' data-in-vendor-specsheet='" . $r['data_in_vendor_specsheet'] . "'>
							<td> <i class='fa " . ($r['visible'] == 'Y' ? 'fa-eye text-success' : 'fa-eye-slash text-danger') . "' aria-hidden='true'></i> </td>
							...
							<td class='align-middle'><i class='fa fa-trash btnDeleteRow pull-right' aria-hidden='true'></i></td>
						</tr>
						";
					}
