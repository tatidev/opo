...
class Product extends MY_Controller
{
	...
	public function get_products()
	{
		...
		//pkl_test = $this->return_datatables_data($list['arr'], $list)
...
//				unset($aux);
				...
				// organize the data for the $files_encoded array
		echo '<pre> PKL TEST LINE:' . __LINE__ . '<br/>	';
		print_r($aux);
		echo '</pre>';	
		exit;
				$aux_d = array(
                ...
                // push the data to the $files_encoded array
				array_push($files_encoded, $aux_d);
				$this->data['list_files']['tbody'] .= "
				<tr>
					<td><a href='" . site_url() . str_replace($_SERVER['DOCUMENT_ROOT'], '', $aux_d['url_dir']) . "' target='_blank'><i class='fa fa-file btnViewFile' aria-hidden='true'></i> " . ($isOther ? $aux_d['descr'] : $aux_d['category_name']) . "</a></td>
					<td>" . $aux_d['date'] . "</td>
					" . ($this->hasPermission('product', 'edit') ? "<td><i class='fa fa-times-circle delete_temp_url' aria-hidden='true'></i></td>" : '') . "
				</tr>";
			}
		}
		// make files_encoded available to the view as json
...
//			  'p_hosp_cut' => empty($this->input->post('p_hosp_cut')) ? null : $this->input->post('p_hosp_cut'),
		...
		*/
				...
				if (count($i->files) > 0) { // It at least one file needs to be entered
					...
					foreach ($i->files as $f) { // Each $f is a URL to the document
						...
						if (strpos($f, 'temp')) {
							...
							$filename = $product_id . '-' . $new_spec_id . '-' . url_title($i->abrasion_limit_name . '-' . $i->rubs . '-' . $i->abrasion_test_name, '-', true) . '-' . $n . '.' . $extension[1];
							...
							// Save new location
							// Rename or move file: The rename() is a codigniter sys function to rename or move a file
				...
				if (count($i->files) > 0) { // It at least one file needs to be entered
					...
					foreach ($i->files as $f) {
						...
						if (strpos($f, 'temp')) {
							// Is a new file!
							$extension = explode('.', str_replace(site_url() . $this->file_directory->temp_folder, '', $f)); // [0] name / [1] extension .jpg or .pdf (whatever)
							$filename = $product_id . '-' . $new_spec_id . '-' . url_title($i->firecode_test_name, '-', true) . '-' . $n . '.' . $extension[1];
        ...
        // Save only if a modification was logged via JS
		if ($this->input->post('change_files_encoded') === '1') { 
			// Product filling
			// If controller called by JS with $_POST['change_files_encoded'] = 1
			// Then we must save the files decode $_POST['files_encoded'] into an array
			...
			foreach ($arr as $i) { 
				...
				if ($i->category_id === $this->category_files_ids['memotags_picture']) {
					...
					if (strpos($f, 'temp')) {
						$extension = explode('.', str_replace(site_url() . $this->file_directory->temp_folder, '', $f)); // [0] name / [1] extension .jpg or .pdf (whatever)
						$filename = $product_id . '-' . url_title($i->category_id . '-' . $i->category_name . '-' . $i->date, '-', true) . '-' . $n . '.' . $extension[1];
						...
						echo "<pre> Product.php: 1.  new_location: $new_location </pre>";
					...
					// $f becomes the Url
					$f = $i->url_dir; 
					// If the URL has "temp" in it, then it is a
					// new uploaded file that needs to be saved
					if (strpos($f, 'temp')) {
						$extension = explode('.', str_replace(site_url() . $this->file_directory->temp_folder, '', $f)); // [0] name / [1] extension .jpg or .pdf (whatever)
						$filename = $product_id . '-' . url_title($i->category_id . '-' . $i->category_name . '-' . $i->date, '-', true) . '-' . $n . '.' . $extension[1];
                        ...
                        // Calculate the image path as $new_location
						$_new_location = $this->file_directory->image_src_path($location_request);
						$new_location = $_new_location . $filename;

						// $new_location  Is a new file location!
						while (file_exists($new_location)) {
							$n++;
							$filename = $product_id . '-' . url_title($i->category_id . '-' . $i->category_name . '-' . $i->date, '-', true) . '-' . $n . '.' . $extension[1];
							$new_location = $_new_location . $filename;
						...
						die();
						// Rename or move file: The rename() is a codigniter sys function to rename or move a file
						// $f is the fresh uploaded url (path) that has "temp" in it
						rename(str_replace(site_url(), '', $f), $new_location);
						// Us FileUploadToS3 to upload the file to S3
						// $this->file_upload_to_s3($new_location, $filename);
					} else {
						// Existing file, don't relocate the file
						$new_location = str_replace(site_url(), '', $f);
					}
                    ...
                    // NOTE: file path/url_dir/location is embeded in the $ret array
					//       which in turn is embeded in the $batch array 
					$ret = array(
					  'product_id' => $product_id,
					  'product_type' => $this->data['product_type'],
					  'category_id' => $i->category_id,
			...
			//var_dump($batch); exit;
			// This saves the files with the path/url_dir/location embeded in the batch[ret]
		...
		if ($this->input->post('change_showcase') === '1') { 
            // $_POST['pic_big_url'] is the URL of the image and has "temp" in the URL
			// Then it is a new image that needs to be saved
			...
			$ret = array(
			  ...
			  'visible' => (is_null($this->input->post('showcase_visible')) ? 'N' : 'Y'),
			...
			if ($this->input->post('change_prices') === '1') { // Save only if a modification was logged via JS
				...
				*/
				...
				$ret = array(
				  'product_id' => $product_id,
				  'product_type' => $this->data['product_type'],
//				  'p_hosp_cut' => empty($this->input->post('p_hosp_cut')) || $this->input->post('p_hosp_cut') === 'NaN' ? null : $this->input->post('p_hosp_cut'),
					'p_hosp_roll' => $this->input->post('p_hosp_roll'),
				  'p_res_cut' => empty($this->input->post('p_res_cut')) || $this->input->post('p_res_cut') === 'NaN' ? null : $this->input->post('p_res_cut'),
				  'user_id' => $user_id
				);
				$this->model->save_price($ret, $product_id, $this->data['product_type']);
			}
