...
//					  'user_id' => $aux[2],
							...
							$this->data['list_files']['tbody'] .= "
				<tr>
					<td><a href='" . site_url() . $aux_d['url_dir'] . "' target='_blank'><i class='fa fa-file btnViewFile' aria-hidden='true'></i></a></td>
					<td>" . $aux_d['date'] . "</td>
			...
			if ($given_style_type === constant('Digital')) {
				...
				if (!empty($files)) {
					foreach ($files as $d) {
						...
						$data['list_files']['tbody'] .= "
					<tr>
						<td><a href='" . site_url() . $aux_d['url_dir'] . "' target='_blank'><i class='fa fa-file btnViewFile' aria-hidden='true'></i> " . ($isOther ? $aux_d['descr'] : $aux_d['category_name']) . "</a></td>
						<td>" . $aux_d['date'] . "</td>
						" . ($this->hasPermission('specs', 'edit') ? "<td><i class='fa fa-times-circle delete_temp_url' aria-hidden='true'></i></td>" : '') . "
					</tr>";
					}
				...
				if ($this->input->post('change_showcase') === '1') {
					...
					if (strpos($f, 'temp')) {
						...
						while (file_exists($temp)) {
							// Move existing file
							$temp = str_replace('.' . $extension[1], '-' . url_title(date("Y-m-d h-i-sa")) . '.' . $extension[1], $new_location);
						}
						$new_location = $temp;
						rename($f, $new_location);
						...
						if ($isNew) {
							...
							if (strpos($f, 'temp')) {
								...
								while (file_exists($temp)) {
									// Move existing file
									$location_request['item_id'] = strval($id) . '-' . url_title(date("Y-m-d h-i-sa"));
									$temp = $this->file_directory->image_src_path($location_request);
//								rename($new_location, $this->file_directory->image_src_path($location_request));
//								$location_request['item_id'] = $id;
								}
								$new_location = $temp;
				...
				case 'abrasion':
					...
					if (!empty($arr)) {
						foreach ($arr as $r) {
							...
							if (count($files) > 0) {
								foreach ($files as $f) {
									$filesanchors .= "<a href='" . site_url() . str_replace(site_url(), '', $f) . "' target='_blank'><i class='fa fa-file btnViewFile' aria-hidden='true'></i></a> ";
								}
							}
							$data['tbody'] .= "
