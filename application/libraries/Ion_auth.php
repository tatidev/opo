...
class Ion_auth
{
	 ...
	 * __call
	 *
	 * Acts as a simple way to call model methods without loads of stuproduct_id alias'
	 *
	 * @param $method
	 * @param $arguments
	 * @return mixed
		...
		if ($this->ion_auth_model->forgotten_password($identity))   //changed
		{
			...
			if ($user) {
				...
				if (!$this->config->item('use_ci_email', 'ion_auth')) {
					...
					$fromEmail = "";
					$fromEmail_title = "";
					$fromEmail .= $this->config->item('admin_email', 'ion_auth');
					$fromEmail_title .= $this->config->item('site_title', 'ion_auth');
					$subject = $this->config->item('site_title', 'ion_auth') . ' - ' . $this->lang->line('email_forgotten_password_subject');

  

					$response =	$this->sendForgotPasswordMail([$user->email], $subject, $fromEmail, $fromEmail_title, $message);
					if ($response) {
						$this->set_message('forgot_password_successful');
						return TRUE;
					} else {
	...
	function sendForgotPasswordMail($mails, $subject, $fromEmail, $fromEmail_title, $message)
	{
		$cfg = require_once(APPPATH . 'core/phpMailer_config.php');
		...
		$email->isSMTP();
		$email->SMTPAuth = true;
		$email->Username = $cfg['username'];
		$email->Password = $cfg['password'];
		$email->SMTPSecure = $cfg['encryption'];
		$email->Host     = $cfg['host'];
		$email->Port     = $cfg['port'];
		$email->SMTPDebug = 0;
		$email->setFrom($fromEmail, $fromEmail_title);
		foreach ($mails as $receiverEmail) {
			$email->addAddress($receiverEmail);
		}
	...
	} // end of function sendOrderEmail
	 ...
	 * @return bool
	 */
	public function register($identity, $password, $email, $additional_data = array(), $group_ids = array(), $showroom_ids = array()) //need to test email activation
	{
		$this->ion_auth_model->trigger_events('pre_account_creation');

		$email_activation = $this->config->item('email_activation', 'ion_auth');

		$id = $this->ion_auth_model->register($identity, $password, $email, $additional_data, $group_ids, $showroom_ids);
	...
	*/
	...
	{
		return $this->ion_auth_model->getPermissionsArr();
	}
}
