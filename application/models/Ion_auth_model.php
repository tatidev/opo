...
class Ion_auth_model extends MY_Model
{
	...
	{
		parent::__construct();
		//$this->auth_db->db_select($this->auth_db_users);

		$this->auth_db = $this->load->database('authentication', TRUE);
	...
	public function hash_password_db($id, $password, $use_sha1_override = FALSE)
	{
		...
		$query = $this->auth_db->select('password, salt')
		  ->where('id', $id)
		  ->limit(1)
		  ->order_by('id', 'desc')
	...
	public function activate($id, $code = false)
	{
		...
		if ($code !== FALSE) {
			$query = $this->auth_db->select($this->identity_column)
			...
			$this->trigger_events('extra_where');
			$this->auth_db->update($this->tables['users'], $data, array('id' => $id));
		} else {
			$data = array(
			  'activation_code' => NULL,
			  'active' => 1
		...
		$return = $this->auth_db->affected_rows() == 1;
		if ($return) {
	...
	public function clear_forgotten_password_code($code)
	{
		...
		$this->auth_db->where('forgotten_password_code', $code);

		if ($this->auth_db->count_all_results($this->tables['users']) > 0) {
			$data = array(
			  'forgotten_password_code' => NULL,
			  'forgotten_password_time' => NULL
			);

			$this->auth_db->update($this->tables['users'], $data, array('forgotten_password_code' => $code));
	...
	public function reset_password($identity, $new)
	{
		...
		$query = $this->auth_db->select('id, password, salt')
		...
		$this->trigger_events('extra_where');
		$this->auth_db->update($this->tables['users'], $data, array($this->identity_column => $identity));
	...
	public function change_password($identity, $old, $new)
	{
		...
		if ($old_password_matches === TRUE) {
			...
			$successfully_changed_password_in_db = $this->auth_db->update($this->tables['users'], $data, array($this->identity_column => $identity));
	...
	public function username_check($username = '')
	{
		...
		return $this->auth_db->where('username', $username)
			->group_by("id")
			->order_by("id", "ASC")
			->limit(1)
	...
	public function email_check($email = '')
	{
		...
		return $this->auth_db->where('email', $email)
			->group_by("id")
			->order_by("id", "ASC")
			->limit(1)
			->count_all_results($this->tables['users']) > 0;
	...
	public function identity_check($identity = '')
	{
		...
		return $this->auth_db->where($this->identity_column, $identity)
			->count_all_results($this->tables['users']) > 0;
	}
	...
	public function forgotten_password($identity)
	{
		...
		$this->auth_db->update($this->tables['users'], $update, array($this->identity_column => $identity));
	 ...
	 * @author Mathew
	 **/
	public function register($identity, $password, $email, $additional_data = array(), $groups = array(), $showrooms = array())
	{
		...
		$query = $this->auth_db->get_where($this->tables['groups'], array('name' => $this->config->item('default_group', 'ion_auth')), 1)->row();
		...
		$this->auth_db->insert($this->tables['users'], $user_data);

		$id = $this->auth_db->insert_id($this->tables['users'] . '_id_seq');
		...
		if (!empty($showrooms)) {
			// add user to showroom
			$batch = array();
			foreach ($showrooms as $showroom_id) {
				array_push($batch, array('user_id' => $id, 'showroom_id' => $showroom_id));
			}
			$this->auth_db->insert_batch($this->tables['users_showrooms'], $batch);
		}
	...
	public function login($identity, $password, $remember = FALSE)
	{
		...
		$query = $this->auth_db
		  ->select($this->identity_column . ", email, " . $this->tables['users'] . ".id, password, active, last_login, GROUP_CONCAT(" . $this->tables['users_groups'] . ".group_id) as `groups`")
	...
	{
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			$this->auth_db->select('1', FALSE);
			$this->auth_db->where('login', $identity);
			if ($this->config->item('track_login_ip_address', 'ion_auth')) {
				if (!isset($ip_address)) {
					$ip_address = $this->_prepare_ip($this->input->ip_address());
				}
				$this->auth_db->where('ip_address', $ip_address);
			}
			$this->auth_db->where('time >', time() - $this->config->item('lockout_time', 'ion_auth'), FALSE);
			$qres = $this->auth_db->get($this->tables['login_attempts']);
	...
	public function get_last_attempt_time($identity, $ip_address = NULL)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			...
			$this->auth_db->order_by('id', 'desc');
			$qres = $this->auth_db->get($this->tables['login_attempts'], 1);
	...
	public function get_last_attempt_ip($identity)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth') && $this->config->item('track_login_ip_address', 'ion_auth')) {
			$this->auth_db->select('ip_address');
			$this->auth_db->where('login', $identity);
			$this->auth_db->order_by('id', 'desc');
			$qres = $this->auth_db->get($this->tables['login_attempts'], 1);
	...
	public function increase_login_attempts($identity)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			...
			return $this->auth_db->insert($this->tables['login_attempts'], $data);
		}
		return FALSE;
	}
	...
	public function clear_login_attempts($identity, $old_attempts_expire_period = 86400, $ip_address = NULL)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			...
			$this->auth_db->or_where('time <', time() - $old_attempts_expire_period, FALSE);

			return $this->auth_db->delete($this->tables['login_attempts']);
		}
		return FALSE;
	}
	...
	public function users($groups = NULL)
	{
		...
		if (isset($this->_ion_select) && !empty($this->_ion_select)) {
			foreach ($this->_ion_select as $select) {
				$this->auth_db->select($select);
			}

			$this->_ion_select = array();
		} else {
			//default selects
			$this->auth_db->select(array(
			  $this->tables['users'] . '.*',
		...
		if (isset($groups)) {
			...
			if (!empty($group_names)) {
				$this->auth_db->join($this->tables['groups'], $this->tables['users_groups'] . '.' . $this->join['groups'] . ' = ' . $this->tables['groups'] . '.id', 'inner');
				$this->auth_db->where_in($this->tables['groups'] . '.name', $group_names);
			}
			if (!empty($group_ids)) {
				$this->auth_db->{$or_where_in}($this->tables['users_groups'] . '.' . $this->join['groups'], $group_ids);
			}
		}
		...
		if (isset($this->_ion_like) && !empty($this->_ion_like)) {
			foreach ($this->_ion_like as $like) {
				$this->auth_db->or_like($like['like'], $like['value'], $like['position']);
			}
		...
		if (isset($this->_ion_limit) && isset($this->_ion_offset)) {
			$this->auth_db->limit($this->_ion_limit, $this->_ion_offset);

			$this->_ion_limit = NULL;
			$this->_ion_offset = NULL;
		} else if (isset($this->_ion_limit)) {
			$this->auth_db->limit($this->_ion_limit);
		...
		if (isset($this->_ion_order_by) && isset($this->_ion_order)) {
			$this->auth_db->order_by($this->_ion_order_by, $this->_ion_order);

			$this->_ion_order = NULL;
			$this->_ion_order_by = NULL;
		}

		$this->response = $this->auth_db->get($this->tables['users']);
	...
	public function get_users_groups($id = FALSE)
	{
		...
		return $this->auth_db->select($this->tables['users_groups'] . '.' . $this->join['groups'] . ' as id, ' . $this->tables['groups'] . '.name, ' . $this->tables['groups'] . '.description')
		  ->where($this->tables['users_groups'] . '.' . $this->join['users'], $id)
		  ->join($this->tables['groups'], $this->tables['users_groups'] . '.' . $this->join['groups'] . '=' . $this->tables['groups'] . '.id')
		  ->get($this->tables['users_groups']);
	}

	public function get_users_showrooms($id = FALSE)
	{
		// if no id was passed use the current users id
		$p_showroom = $this->db_thismaster . '.' . $this->p_showroom;
		$id || $id = $this->session->userdata('user_id');

		return $this->auth_db->select($this->tables['users_showrooms'] . ".showroom_id as id, $p_showroom.name, $p_showroom.abrev as description")
		  ->where($this->tables['users_showrooms'] . '.user_id', $id)
		  ->join($p_showroom, $this->tables['users_showrooms'] . ".showroom_id = $p_showroom.id")
		  ->get($this->tables['users_showrooms']);
	}

	public function get_users_restock_destination($id = FALSE)
	{
		$p_restock_destination = $this->db_thismaster . '.' . $this->p_showroom;
		$id || $id = $this->session->userdata('user_id');

		return $this->auth_db
		  ->select($this->tables['users_destinations'] . ".restock_destination_id as id, $p_restock_destination.name")
		  ->where($this->tables['users_destinations'] . '.user_id', $id)
		  ->join($p_restock_destination, $this->tables['users_destinations'] . ".restock_destination_id = $p_restock_destination.id")
		  ->get($this->tables['users_destinations']);
	}
	...
	public function add_to_group($group_ids, $user_id = false)
	{
		...
		foreach ($group_ids as $group_id) {
			if ($this->auth_db->insert($this->tables['users_groups'], array($this->join['groups'] => (float)$group_id, $this->join['users'] => (float)$user_id))) {
	...
	public function remove_from_group($group_ids = false, $user_id = false)
	{
		...
		if (!empty($group_ids)) {
			...
			foreach ($group_ids as $group_id) {
				$this->auth_db->delete($this->tables['users_groups'], array($this->join['groups'] => (float)$group_id, $this->join['users'] => (float)$user_id));
			...
			if ($return = $this->auth_db->delete($this->tables['users_groups'], array($this->join['users'] => (float)$user_id))) {
	...
	public function groups()
	{
		...
		$this->auth_db->order_by('name', 'asc');
		$this->response = $this->auth_db->get($this->tables['groups']);
	...
	public function update($id, array $data)
	{
		...
		$this->auth_db->trans_begin();

		if (array_key_exists($this->identity_column, $data) && $this->identity_check($data[$this->identity_column]) && $user->{$this->identity_column} !== $data[$this->identity_column]) {
			$this->auth_db->trans_rollback();
			$this->set_error('account_creation_duplicate_identity');
		...
		$this->trigger_events('extra_where');
		$this->auth_db->update($this->tables['users'], $data, array('id' => $user->id));

		if ($this->auth_db->trans_status() === FALSE) {
			$this->auth_db->trans_rollback();
	...
	public function delete_user($id)
	{
		...
		$this->auth_db->delete($this->tables['users'], array('id' => $id));
	...
	public function update_last_login($id)
	{
		...
		$this->auth_db->update($this->tables['users'], array('last_login' => time()), array('id' => $id));
	...
	public function remember_user($id)
	{
		...
		$this->auth_db->update($this->tables['users'], array('remember_code' => $salt), array('id' => $id));

		if ($this->auth_db->affected_rows() > -1) {
			// if the user_expire is set to zero we'll set the expiration two years from now.
			if ($this->config->item('user_expire', 'ion_auth') === 0) {
				$expire = (60 * 60 * 24 * 365 * 2);
			} // otherwise use what is set
	...
	public function login_remembered_user()
	{
		...
		// get the user
        // ------------------------------------------------------------------
        //  Get the user who has a valid “remember me” cookie
        // ------------------------------------------------------------------
        $this->trigger_events('extra_where');
        
        $query = $this->auth_db
            ->select(
                $this->identity_column . '                 AS username, ' .        // e.g. auth_users.username
                $this->tables['users']   . '.id, ' .
                $this->tables['users']   . '.email, ' .
                $this->tables['users']   . '.last_login, ' .
                'GROUP_CONCAT(' . $this->tables['users_groups'] . '.group_id) AS user_groups'  // avoid the keyword “GROUPS”
            )
            ->join(
                $this->tables['users_groups'],
                $this->tables['users'] . '.id = ' . $this->tables['users_groups'] . '.user_id'
            )
            ->where(
                $this->identity_column,
                urldecode(get_cookie($this->config->item('identity_cookie_name', 'ion_auth')))
            )
            ->where(
                $this->tables['users'] . '.remember_code',
                get_cookie($this->config->item('remember_cookie_name', 'ion_auth'))
            )
            ->where($this->tables['users'] . '.active', 1)
            // --- NEW: satisfy ONLY_FULL_GROUP_BY ---
            ->group_by([
                $this->tables['users'] . '.id',
                $this->identity_column,
                $this->tables['users'] . '.email',
	...
	public function create_group($group_name = FALSE, $group_description = '', $additional_data = array())
	{
		...
		$existing_group = $this->auth_db->get_where($this->tables['groups'], array('name' => $group_name))->num_rows();
		if ($existing_group !== 0) {
		...
		$this->auth_db->insert($this->tables['groups'], $data);
		$group_id = $this->auth_db->insert_id($this->tables['groups'] . '_id_seq');
		...
		$this->auth_db
			...
			->delete($this->tables['groups_permissions']);
		*/
		// Now insert new permissions posted
		$batch = array();
		if (!empty($permissions)) {
			...
			$this->auth_db->insert_batch($this->tables['groups_permissions'], $batch);
		}
	 ...
	 * @author aditya menon
	 **/
	public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array(), $permissions = array())
	{
		...
		if (!empty($group_name)) {
			...
			// bail if the group name already exists
			$existing_group = $this->auth_db->get_where($this->tables['groups'], array('name' => $group_name))->row();
		...
		$group = $this->auth_db->get_where($this->tables['groups'], array('id' => $group_id))->row();
		...
		$this->auth_db->update($this->tables['groups'], $data, array('id' => $group_id));
	...
	public function delete_group($group_id = FALSE)
	{
		...
		$this->auth_db->delete($this->tables['users_groups'], array($this->join['groups'] => $group_id));
		// remove the group itself
		$this->auth_db->delete($this->tables['groups'], array('id' => $group_id));
	...
	{
		$filtered_data = array();
		$columns = $this->auth_db->list_fields($table);
	...
	*/
	function getPermissionsArr()
	{
		$user_id = $this->session->userdata('user_id');
		$group_ids = explode(',', $this->session->userdata('groups'));

		if (in_array('1', $group_ids)) {
			return $this->getAllPermissions();
		}

		$this->auth_db->select("D.module, D.action")
		  ->distinct()
		  ->from($this->tables['users'] . ' A')
		  ->join($this->tables['users_groups'] . ' B', 'A.id = B.user_id')
		  ->join($this->tables['groups_permissions'] . ' C', 'B.group_id = C.group_id')
		  ->join($this->tables['permissions'] . ' D', 'C.permission_id = D.id')
		  ->where('A.id', $user_id)
		  ->where('D.app', 'master')
		  ->order_by('D.module, D.action');

		return $this->auth_db->get()->result_array();
	}
	...
	*/
	...
	{
		$this->auth_db->select("id, app, module, action")
		  ->distinct()
		  ->from($this->tables['permissions'] . ' A')
			->where('app', 'master')
		  ->order_by('app, module, action');
	...
	*/
	...
	{
		$this->auth_db->select("D.id, D.app, D.module, D.action")
		  ->distinct()
		  ->from($this->tables['groups_permissions'] . ' A')
		  ->join($this->tables['permissions'] . ' D', 'A.permission_id = D.id')
		  ->where('A.group_id', $id)
		  ->order_by('D.module, D.action');
