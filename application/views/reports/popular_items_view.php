            ...
            <div id='filter_row' class='row d-flex flex-row mx-auto'>
				...
				<?php foreach ($filters as $f) { ?>
                    <div class='filter-group'>
                        <div class='row'>
                            <label for="<?php echo  url_title($f['field_name']) ?>" class="col-xs-12 col-sm-4 col-form-label"
            ...
            .DataTable({
                ...
                "columns": [
                    ...
                    },
                    {
                        ...
                        "render": function (data, type, row, meta) {
                            var add = '';
                            if ('item_id' in row) attr = 'item_id';
                            else if ('product_id' in row) attr = 'product_id';
                            else console.log('no product type recognized...');
                            console.log(attr);
                            switch (row.product_type) {
                                case 'R':
                                    if (stamps.under30_ids.indexOf(row[attr]) >= 0) add += get_stamp_html('30U');
                                    if (stamps.digital_ground_ids.indexOf(row[attr]) >= 0) add += get_stamp_html('DG');
                                    if (stamps.fabricseen_ids.indexOf(row[attr]) >= 0) add += get_stamp_html('FS');
                                    break;
                            }
                            return add;
                        }
                    ...
                    {
                        "title": "Total Samples Ordered",
                        "data": "total_samples_ordered"
                    }
                ],
                "order": [
                    [0, "desc"]
