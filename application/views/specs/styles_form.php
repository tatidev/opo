	...
	<div class='row p-4'>
		...
		<div class='col-12'>
			<div class='row'>
				...
				<div class='col-6'>
					<?php echo ( $hasPermission ? ' <a class="btn btn-success float-right btnSave btnSaveStyle"><i class="far fa-square" aria-hidden="true"></i> Save</a> ' : '')?>
				</div>
			</div>
			<h3 class='mt-4'>
				Pattern Edit
...
<? if( $style_type === constant('Digital') ){ ?>
  ...
  <div class='col-12'>
    ...
    <div class="row">
      ...
      <? if ($hasPermission) { ?>
      <div class='col'>
        <span class="btn btn-link" data-for="pfiles" onclick="javascript: upload_product_file(this)">Upload new file <i class="fa fa-plus" aria-hidden="true"></i></span>
      </div>
      <? } ?>
    </div>
...
<? } ?>
  ...
  <form id='fileupload_product' action='<?php echo site_url('specs/uploadToTemp')?>' method='POST'  enctype='multipart/form-data'>
...
<script>
	...
	$(document).ready(function(){
		var items = JSON.parse('<?//=$items_list['json']?>');
		$('#style_items_datatable').DataTable({
			"data": items,
      "pageLength": 50,
      "columns": [
        { "title": "", "searchable": false, "orderable": false, "className": "no-export",
          "render": function ( data, type, row, meta ) {
            return "<i data-toggle='tooltip' data-trigger='hover' data-title='Edit Color' data-placement='top' class='fas fa-pen-square btn-action btnEditStyleItem' data-item-id='"+row.id+"'></i>";
          }
				},
        { "title": "Web Visible", "searchable": false, "orderable": false,
          "render": function ( data, type, row, meta ) {
            return "<label class='custom-control custom-checkbox col-form-label'> \
											<input type='checkbox' id='showcase_visible_"+row.id+"' name='showcase_visible_"+row.id+"' class='custom-control-input form-control' "+( row.showcase_visible === 'Y' ? "checked" : "" )+"> \
											<span class='custom-control-indicator'></span> \
										</label>";
          }
				},
        { "title": "Color", "searchable": false, "orderable": false, "className": "no-export",
          "render": function ( data, type, row, meta ) {
            return ""+row.color_names+" \
							<input type='hidden' id='color_ids_"+row.id+"' name='color_ids_"+row.id+"' value='"+JSON.stringify( row.color_ids.split(' / ') )+"'> \
							<input type='hidden' id='color_names_"+row.id+"' name='color_names_"+row.id+"' value='"+JSON.stringify( row.color_names.split(' / ') )+"'>";
          } 
				},
        { "title": "Thumbnail (400x400)",
          "render": function ( data, type, row, meta ) {
						return 				"							<div class='img-thumbnail-container'> \
								<a onclick=\"window.open( $(this).children().attr('src') )\"> \
									<img id='img_pic_big_url_"+row.id+"' src='"+row.pic_big_url+"' class='img-thumbnail'> \
									<input type='hidden' class='form-control' id='pic_big_url_"+row.id+"' name='pic_big_url_"+row.id+"' value='"+row.pic_big_url+"'> \
								</a> \
							</div> \
							<span class='btn btn-link' data-for='pic_big_url_"+row.id+"' onclick=\"javascript: upload_product_file(this);\"> \
								"+ ( row.pic_big_url !== '' ? 'Replace Image' : 'Upload Image') +" \
								<i class='fa fa-plus' aria-hidden='true'></i> \
							</span>";
...
<script>
	...
	function upload_product_file(me) {
		...
		console.log('function upload_product_file(me) ', $(me).attr('data-for'));
			...
			switch ($(me).attr('data-for')) {
                ...
                default:
				    console.log('upload_product_file(me) default ', $(me));
					$('#pfiles').attr('multiple', null).attr('name', 'files');
  ...
  $(document).ready(function() {
	...
	// fileupload is from Blueimp
  	$('#fileupload_product').fileupload({
  		...
  		formData: function(e, data) {
  			...
  			$.each(data.result.files, function(index, file) {
					if( file.category_id === 'colorway' ){
						console.log(' style_form uplod() file', file);
		  ...
		  console.log('$(#fileupload_product).fileupload(');
  			});
  		}
  	});
  	...
  	function add_file_list(file) {
  	  ...
  	  var new_row = $("<tr><td><a href='" + file.url + "' target='_blank'><i class='fas fa-file' aria-hidden='true'></i> " + file_category_name + "</a></td><td>" + now + "</td> <td><i class='fas fa-times-circle delete_temp_url' aria-hidden='true'></td></tr>");
  	  ...
  	  aux.push(ne);
	  console.log("function add_file_list(file)  files_encoded ", aux);
