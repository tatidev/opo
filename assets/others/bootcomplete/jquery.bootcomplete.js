            ...
            if(Data[settings.queryKey].length >= settings.minLength){
                ...
                xhr = $.ajax({
                    ...
                    success: function( json ) {
                        var results = ''
												var count_results = json.length;
											
												if( count_results === 0 && $('input#new_color').val().length > 0 ){
													 // No result
													$('i#btnNewColor').parent().removeClass('hide');
													
												} else {
													$('i#btnNewColor').parent().addClass('hide');
        ...
        function selectResult(){
            ...
            if (settings.idField) {
                if ($(that).parent().parent().find('input[name="' + settings.idFieldName + '"]').length !== 0) {
                    ...
                    $(that).parent().parent().find('input[name="' + settings.idFieldName + '"]').val($(this).data('id')).trigger('change');
                }
                else {
                    //use created id field
                    $(that).prev('input[name="' + settings.idFieldName + '"]').val($(this).data('id')).trigger('change');
                }
            }
            $(that).next('.' + settings.menuClass).hide();
            return false;
